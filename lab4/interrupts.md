...
*  RISC-V: The address of the instruction that caused the exception/ that was about to be executed when an interrupt came in is saved into epc (exception program counter, and not ra). Since we aren't aiming for compliance, saving automatically to another register that isn't used in your program is an option too. Saving to `ra` can be problematic when you have function calls as `ra` gets overwritten. You can then use `jalr` to return from interrupt. 
