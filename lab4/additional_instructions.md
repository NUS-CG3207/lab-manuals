...
* Add support for the previously unsupported DP instructions: `xor`, `xori`, `slt`, `sltu`, `slti`, `sltiu`, `slli`, `srli`, `srai`. 

* Add support for all the previously unsupported branch instructions: `blt`, `bge`, `bltu`, `bgeu`, `jal` (with support for linking: i.e. `rd = PC + 4`), `jalr`, `ret`. 

* Add support for all multiply instructions: `mulh`, `mulhu`, `mulhsu`, `div`, `rem`, `remu`

* Add support for other ISA extensions: see the [RISC-V Specification](https://riscv.org/wp-content/uploads/2019/12/riscv-spec-20191213.pdf) for ideas. 
