...
*   The provided HelloWorld program is perhaps **NOT** the first assembly language program you should attempt on _your_ processor. Use the **Lab 1 assembly language program** (with appropriate modifications) instead.  The provided test\_Wrapper is meant to test the HelloWorld assembly language program; slight **modifications will be needed** to test other assembly language programs you write. HelloWorld is neither meant to be a program for you to get started, nor meant to be a comprehensive program that tests everything. It is just a fancy example program. 
*   Use your own, well-crafted assembly language programs for a convincing demo (to demonstrate that all the required instructions and variants work). **One single program demonstrating all the features would be desirable**. The purpose of the program you did in Lab 1 is to get you started (i.e., the Lab 1 program working on the board is **not sufficient**). See the tips section below for more info. Also, note that you might need to **modify your Lab 1 program to use only those instructions you have implemented in YOUR processor** (**and simulate it in RARS** to ensure the functionality) before you get started. Also note that most of you have not used **DP reg type** in Lab 1, so you will need to add at least one instruction of that type in your code to test your basic processor.
*   There is no requirement that you should use all the peripherals supported by the Wrapper. As long as your demo is convincing, it is fine to use only a limited set of peripherals (say, LEDs and DIP switches \- at least one input and one output). [RISC-V Memory Map](RISC-V-Memory-Map_409240879.html) page has more details of the address and usage of the supported peripherals.
    *   You can add more peripherals (RGB LED, accelerometer, VGA display etc.) to the Wrapper if you wish. The corresponding changes will also need to be done in the top-level .vhd and .xcf files.

### RISC Programming Instructions

Please follow the instructions in the [RISC-V Programming](RISC-V-Programming_309430718.html) page configure RARS and to write programs. The .hex file generated by the program is inserted into the ROMs within Wrapper.v to "program" the RISC-V processor using the procedure mentioned in that page.
...
*   When you do synthesis / implementation etc, **choosing the number of jobs** to be the max in the drop-down list (number of cores) in the dialog that appears when you run synthesis will reduce the time required. The default is typically not the max.
...
*   You can go into the subunits and see their value for each instruction (Scope-Objects) - this is much easier than what most of you think. This is more powerful than dragging the various signals into the waveform.
...
*   You should have your own programs to have a convincing demo (to demonstrate that all the required instructions and variants work). By 'convincing demo', what we mean is having an assembly language program that tests all the features of all instructions of a particular type. For example, if you demonstrate **addi**, you don't really have to show **andi**, **ori** as it can be expected to work.  
    Your program should be such that if one instruction misbehaves, the overall behavior of the program should be different (this is the case for most programs, as long as you use the result from every instruction in a subsequent instruction).
